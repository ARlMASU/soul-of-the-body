...
export function handleOptionsMenuLanguageSelection() {
    ...
    function changeChoice(choiceIndex) {
        languageChoicesWrapper.style.transform = `translateX(-${choiceIndex * 32}px)`;
    }
...
export function handleInvMenuItemsDisplay(dummyInv) {
    ...
    boxes.forEach((box, index) => {
        if (dummyInv[index].img) {
            const itemIcon = document.createElement("img");
            itemIcon.src = `./assets/images/items/${dummyInv[index].img}`;
            box.textContent = "";
            box.append(itemIcon);
        }
        ...
        box.addEventListener("click", (event) => {
            ...
            descriptionBubbleText.textContent = `${dummyInv[index].name}`;
...
export function handleDiaryMenuTasksAndFindingsDisplay(
    dummyTasks,
    dummyFindings,
) {
